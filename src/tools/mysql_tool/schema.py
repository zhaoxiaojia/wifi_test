...
from typing import Any, Dict, Iterable, List, Optional, Sequence, Tuple, TYPE_CHECKING, Mapping
...
_TABLE_SPECS: Dict[str, TableSpec] = {
    ...
    "test_report": TableSpec(
        columns=(
            ...
            ColumnDefinition("csv_name", "VARCHAR(255)"),
    ...
    "dut": TableSpec(
        ...
        constraints=(
            ...
            TableConstraint(
                "fk_dut_test_report",
                "CONSTRAINT fk_dut_test_report FOREIGN KEY (`test_report_id`) REFERENCES `test_report`(`id`) ON DELETE CASCADE",
            ),
        ),
    ),
    "execution": TableSpec(
    ...
    "lab": TableSpec(
        ...
        constraints=(
            TableConstraint(
                "uq_lab_name",
                "CONSTRAINT uq_lab_name UNIQUE (`lab_name`)",
            ),
        ),
    ),
    "lab_environment": TableSpec(
        ...
        indexes=(
            TableIndex(
                "idx_lab_environment_lab_id",
                "INDEX idx_lab_environment_lab_id (`lab_id`)",
            ),
        ),
        constraints=(
            TableConstraint(
                "uq_lab_environment_lab_id",
                "CONSTRAINT uq_lab_environment_lab_id UNIQUE (`lab_id`)",
            ),
            TableConstraint(
                "fk_lab_environment_lab",
                "CONSTRAINT fk_lab_environment_lab FOREIGN KEY (`lab_id`) REFERENCES `lab`(`id`) ON DELETE CASCADE",
            ),
        ),
    ),
    "compatibility": TableSpec(
        ...
        indexes=(
            TableIndex(
                "idx_compat_test_report",
                "INDEX idx_compat_test_report (`test_report_id`)",
            ),
            TableIndex(
                "idx_compat_router",
        ...
        constraints=(
            TableConstraint(
                "fk_compat_test_report",
                "CONSTRAINT fk_compat_test_report FOREIGN KEY (`test_report_id`) REFERENCES `test_report`(`id`) ON DELETE CASCADE",
            ),
            TableConstraint(
                "fk_compat_router",
                "CONSTRAINT fk_compat_router FOREIGN KEY (`router_id`) REFERENCES `router`(`id`) ON DELETE SET NULL",
            ),
        ),
    ),
    "performance": TableSpec(
        ...
        constraints=(
            TableConstraint(
                ...
                "CONSTRAINT fk_performance_test_report FOREIGN KEY (`test_report_id`) REFERENCES `test_report`(`id`) ON DELETE CASCADE",
            ),
        ),
    ),
    "perf_metric_kv": TableSpec(
        ...
        indexes=(
            TableIndex(
                "idx_kv_test_report", "INDEX idx_kv_test_report (`test_report_id`)"
            ),
            TableIndex(
        ...
        constraints=(
            TableConstraint(
                "fk_kv_test_report",
                "CONSTRAINT fk_kv_test_report FOREIGN KEY (`test_report_id`) REFERENCES `test_report`(`id`) ON DELETE CASCADE",
            ),
        ),
    ),
    "artifact": TableSpec(
...
_VIEW_DEFINITIONS: Dict[str, str] = {
    "v_run_overview": """
        ...
        ) AS agg ON agg.test_report_id = tc.id
    """,
    "v_perf_latest": """
        SELECT
            ranked.id,
        ...
        FROM (
            ...
            LEFT JOIN execution AS ex ON ex.test_report_id = tr.id
        ) AS ranked
        WHERE ranked.rn = 1
    """,
}
...
def get_schema_catalog() -> dict[str, Any]:
    ...
    for name, spec in _TABLE_SPECS.items():
        tables[name] = {
            "columns": [{"name": c.name, "definition": c.definition} for c in spec.columns],
            "indexes": [{"name": i.name, "definition": i.definition} for i in spec.indexes],
            "constraints": [{"name": c.name, "definition": c.definition} for c in spec.constraints],
        }
    return {"tables": tables, "views": dict(_VIEW_DEFINITIONS)}


def render_schema_markdown(catalog: Mapping[str, Any]) -> str:
    tables = catalog.get("tables") if isinstance(catalog.get("tables"), Mapping) else {}
    views = catalog.get("views") if isinstance(catalog.get("views"), Mapping) else {}
    lines: list[str] = []
    ...
    for table_name in sorted(tables.keys()):
        ...
        for col in table.get("columns") or []:
            lines.append(f"- `{col.get('name')}`: {col.get('definition')}")
        lines.append("")
        indexes = table.get("indexes") or []
        if indexes:
            lines.append("**Indexes**")
            for idx in indexes:
                lines.append(f"- `{idx.get('name')}`: {idx.get('definition')}")
            lines.append("")
        constraints = table.get("constraints") or []
        if constraints:
            lines.append("**Constraints**")
            for con in constraints:
                lines.append(f"- `{con.get('name')}`: {con.get('definition')}")
            lines.append("")
    lines.append("## Views")
    for view_name in sorted(views.keys()):
        sql = str(views.get(view_name) or "").strip()
