...
def _apply_grouped_texts(ws: Worksheet, column: int, start_row: int, values: Sequence[float], formatter) -> None:
    for start_idx, end_idx, text in _group_runs(values, formatter):
        ...
        style.set_standard_left_wrap(ws, top, column, text)
        style.apply_cells(
            ws,
            column=column,
            start_row=top + 1,
...
def _set_result_cell(ws: Worksheet, row: int, column: int, threshold: Optional[Tuple[int, int]], rx_column: int, tx_column: int) -> None:
    ...
    style.set_body_center(ws, row, column, value)
...
def _write_cover_remark_row(
    ...
    *,
    ...
    style.apply_report_remark(ws, row=row, end_col_letter=last_col, text=remark_text)


def _write_cover_summary_row(
    ws: Worksheet,
    *,
    groups: Sequence[ScenarioGroup],
    test_type: str,
    ...
    style.apply_report_summary(ws, row=row, end_col_letter=last_col, text=summary_text)
...
def _write_report_title(
    ...
    *,
    ...
    top_row = start_row
    last_col = last_column or REPORT_LAST_COLUMN
    _write_cover_title_row(ws, test_type=test_type, row=top_row, last_column=last_col)

    remark_row = top_row + 1
    _write_cover_remark_row(ws, row=remark_row, last_column=last_col, remarks=remarks)

    summary_row = remark_row + 1
    _write_cover_summary_row(
        ws,
        groups=groups,
        test_type=test_type,
        row=summary_row,
...
def _write_group_header(ws: Worksheet, group: ScenarioGroup, start_row: int, *, last_column: str) -> int:
    last_col = last_column or REPORT_LAST_COLUMN
    style.apply_section_header(ws, row=start_row, end_col_letter=last_col, text=group.summary_label)
...
def _write_headers(
    ...
    *,
    ...
    for row, col, text in base_headers:
        style.set_brand_header(ws, row, col, text, wrap=True)
    ...
    def _write_header_block(columns: Sequence[int], label: str) -> None:
        ...
        style.apply_header_block(ws, row=header_row, start_col=start, end_col=end, label=label, wrap=True)
    ...
    if rssi_columns:
        ...
        style.set_brand_header(ws, header_row, layout.rssi_start_col, "RSSI", wrap=True)

    style.set_subheader(ws, sub_row, 1, step_text)
    style.set_subheader(ws, sub_row, layout.rssi_start_col, step_text)

    for idx, scenario in enumerate(channels):
        channel_text = scenario.channel
        if idx < len(layout.rx_cols):
            style.set_subheader(ws, sub_row, layout.rx_cols[idx], channel_text)
        if idx < len(layout.tx_cols):
            style.set_subheader(ws, sub_row, layout.tx_cols[idx], channel_text)

    style.set_subheader(ws, sub_row, layout.aml_standard_col, None)
    style.set_subheader(ws, sub_row, layout.aml_result_col, None)

    for idx, scenario in enumerate(channels):
        channel_text = scenario.channel
        if idx < len(layout.right_rx_cols):
            style.set_subheader(ws, sub_row, layout.right_rx_cols[idx], channel_text)
        if idx < len(layout.right_tx_cols):
            style.set_subheader(ws, sub_row, layout.right_tx_cols[idx], channel_text)
    LOGGER.info(
...
def _write_data(
    ...
    *,
    ...
    style.set_brand_header(ws, start_row, 1, group.summary_label, wrap=False)
    ...
    for index, attenuation in enumerate(steps):
        ...
        for channel_idx, scenario in enumerate(channels):
            ...
            style.set_body_center(ws, row, rx_col, rx_value, fill=highlight)
            style.set_body_center(ws, row, tx_col, tx_value, fill=highlight)

        style.set_standard_left_wrap(ws, row, layout.aml_standard_col, None)
        style.set_body_center(ws, row, layout.aml_result_col, None)
    ...
    *,
    ...
    def _write_header_block(top_row: int, *, throughput_title: str, rssi_title: str, include_item: bool) -> None:
        ...
        for col, text in headers:
            ...
            style.set_brand_header_border(ws, top_row, col, text, border=border_style, wrap=True)
        if angle_count:
            style.apply_header_block(
                ws,
                row=top_row,
                start_col=throughput_start_col,
                end_col=throughput_end_col,
                label=throughput_title,
                wrap=True,
            )
        else:
            style.set_brand_header(ws, top_row, throughput_start_col, throughput_title, wrap=True)
        for idx, angle in enumerate(angle_headers):
            style.set_subheader_fill(
                ws,
                sub_row,
        ...
        if angle_count:
            style.apply_header_block(
                ws,
                row=top_row,
                start_col=rssi_start_col,
                end_col=rssi_end_col,
                label=rssi_title,
                wrap=True,
            )
        else:
            style.set_brand_header(ws, top_row, rssi_start_col, rssi_title, wrap=True)
        for idx, angle in enumerate(angle_headers):
            style.set_subheader_fill(
                ws,
                sub_row,
    ...
    def _write_rx_row(row_index: int, scenario: ProjectScenario, ctx: dict[str, object]) -> None:
        ...
        style.set_body_center(ws, row_index, channel_col, scenario.channel)
        style.set_body_center(ws, row_index, left_att_col, att_display)
        for idx, angle in enumerate(angle_headers):
            col = throughput_start_col + idx
            style.set_body_center(ws, row_index, col, rx_angle_map.get(angle), fill=COLOR_RATE_PRIMARY)
        if angle_count:
        ...
        style.set_body_number(ws, row_index, average_col, rx_average_formula, number_format="0.00")
        style.set_body_number(ws, row_index, ovality_col, rx_ovality_formula, number_format="0.00%")
        style.set_standard_left_wrap(ws, row_index, aml_standard_col, None)
        style.set_body_center(ws, row_index, aml_result_col, None)
        style.set_body_center(ws, row_index, right_att_col, att_display)
        for idx, angle in enumerate(angle_headers):
            col = rssi_start_col + idx
            style.set_body_center(ws, row_index, col, rssi_rx_map.get(angle), fill=COLOR_RSSI_RX)
    ...
    def _write_tx_row(row_index: int, scenario: ProjectScenario, ctx: dict[str, object]) -> None:
        ...
        for idx, angle in enumerate(angle_headers):
            col = throughput_start_col + idx
            style.set_body_center(ws, row_index, col, tx_angle_map.get(angle), fill=COLOR_RATE_PRIMARY)
        if angle_count:
        ...
        style.set_body_number(ws, row_index, average_col, tx_average_formula, number_format="0.00")
        style.set_body_number(ws, row_index, ovality_col, tx_ovality_formula, number_format="0.00%")
        style.set_standard_left_wrap(ws, row_index, aml_standard_col, None)
        style.set_body_center(ws, row_index, aml_result_col, None)
        style.set_body_center(ws, row_index, right_att_col, att_display)
        for idx, angle in enumerate(angle_headers):
            col = rssi_start_col + idx
            style.set_body_center(ws, row_index, col, rssi_tx_map.get(angle), fill=COLOR_RSSI_TX)
    ...
    for scenario in group.channels:
        ...
        if scenario_start <= scenario_end:
            ...
            style.set_body_center(ws, scenario_start, channel_col, scenario.channel)
            style.apply_cells(
                ws,
                column=channel_col,
                start_row=scenario_start + 1,
                end_row=scenario_end,
                border=True,
    ...
    if first_data_row is not None and last_data_row is not None and first_data_row <= last_data_row:
        ...
        style.set_body_center_wrap(ws, group_label_start, item_col, group.summary_label or group.title)
...
def _add_rvo_polar_chart(
    ...
    *,
    ...
    if dataframe is None or getattr(dataframe, "empty", True):
        ...
        style.set_body_center(ws, anchor_row, placeholder_col, "Polar chart unavailable")
    ...
    try:
        ...
        if layout is not None and getattr(layout, "rx_cols", None):
            ...
            fallback_col = 2
        style.set_body_center(ws, anchor_row, fallback_col, "Polar chart unavailable")
...
class _ProjectReportContext:
    actual_type: str
    report_last_column_letter: str


@dataclass(frozen=True)
class _StrategyRuntime:
    """Strategy-specific runtime resources prepared once per report build."""

    report_last_column_letter: str


class _ReportStrategy:
    """Strategy interface for report-type specific behavior."""
...
class _RvoReportStrategy(_ReportStrategy):
    ...
    def __init__(self) -> None:
        ...
        self._chart_context: Optional[dict[str, Any]] = None
    ...
    def prepare(
        ...
        *,
        ...
        self._rvo_att_entries = _resolve_rvo_att_steps()
        self._chart_context = _prepare_rvo_chart_context(result_file)
        if groups:
            report_last_col_index = _determine_report_last_column(
                groups,
                actual_type,
                rvo_att_entries=self._rvo_att_entries,
            )
        else:
            report_last_col_index = column_index_from_string("M")
        report_last_col_index = max(1, int(report_last_col_index))
        return _StrategyRuntime(report_last_column_letter=get_column_letter(report_last_col_index))
    ...
    def build_group_tables(
        ...
        *,
        ...
        override_steps = [entry[0] for entry in self._rvo_att_entries]
        att_labels = [entry[2] for entry in self._rvo_att_entries]
        item_labels = [entry[1] for entry in self._rvo_att_entries]
        angles, entries = _prepare_rvo_table_entries(
            group,
            override_steps,
            att_labels,
    ...
    def build_group_charts(
        ...
        *,
        ...
        base_next_row = table_result.table_end_row + 1
        if self._chart_context is None:
            return base_next_row
        anchor_row = int(table_result.chart_anchor_row or (table_result.table_end_row + 2))
        data_end_col = int(table_result.data_end_col or 1)
        last_row = _add_rvo_polar_chart(
            ...
            data_end_row=int(table_result.table_end_row),
            data_end_col=data_end_col,
        )
        return max(base_next_row, last_row + 6)


class _ThroughputReportStrategy(_ReportStrategy):
    name = "THROUGHPUT"
    ...
    def prepare(
        ...
        *,
        ...
        if groups:
            report_last_col_index = _determine_report_last_column(groups, actual_type, rvo_att_entries=[])
        else:
            report_last_col_index = column_index_from_string("M")
        report_last_col_index = max(1, int(report_last_col_index))
        return _StrategyRuntime(report_last_column_letter=get_column_letter(report_last_col_index))
    ...
    def build_group_charts(
        ...
        *,
        ...
        base_next_row = table_result.table_end_row + 1
        sections = table_result.sections or []
        layout = table_result.layout
        anchor_row = int(table_result.chart_anchor_row or 1)
        if not sections or layout is None:
            LOGGER.warning("Group %s has no channel sections to chart.", group.key)
            return base_next_row
        last_row = _add_group_charts(builder.sheet, group, layout, sections, anchor_row=anchor_row)
        return max(base_next_row, last_row + 6)
...
class ProjectReportBuilder:
    ...
    This is a pragmatic Builder/Template-Method hybrid: the public `build()`
    orchestrates the steps, while individual steps are factored into named
    methods so future changes to formatting/layout can stay localized.
    """
    ...
    def __init__(
        ...
        *,
        ...
        self.forced_test_type = forced_test_type
        self.sheet_name = sheet_name
        self.normalized_type = _normalize_test_type(forced_test_type) if forced_test_type else None

        self.groups: List[ScenarioGroup] = _load_scenario_groups(
            result_file,
    ...
    def _prepare_context(self) -> _ProjectReportContext:
        runtime = self._strategy.prepare(
            result_file=self.result_file,
            groups=self.groups,
            actual_type=self.actual_type,
        )
        return _ProjectReportContext(
            actual_type=self.actual_type,
            report_last_column_letter=runtime.report_last_column_letter,
        )

    def _configure_sheet(self, ctx: _ProjectReportContext) -> None:
        explicit = (self.sheet_name or "").strip()
        if explicit:
            resolved = explicit
        else:
    ...
    def build_cover(self, ctx: _ProjectReportContext) -> None:
        _write_cover_title_row(
            self.sheet,
            test_type=ctx.actual_type,
            row=1,
            last_column=ctx.report_last_column_letter,
        )
        # cover occupies title row; summary step writes the subsequent rows
        self._title_row = 1

    def build_summary(self, ctx: _ProjectReportContext) -> None:
        remark_row = self._title_row + 1
        summary_row = self._title_row + 2
        _write_cover_remark_row(
            self.sheet,
            row=remark_row,
            last_column=ctx.report_last_column_letter,
            remarks=None,
        ...
        self._title_end_row = summary_row

    def build_empty_report(self, ctx: _ProjectReportContext) -> None:
        placeholder_row = self._title_end_row + 2
        style.apply_empty_report_placeholder(
            self.sheet,
            row=placeholder_row,
            end_col_letter=ctx.report_last_column_letter,
            text="No Wi-Fi data available for project report",
        )
        LOGGER.warning("No scenarios found; generated placeholder sheet named %s.", self.sheet.title)

    def build_body(self, ctx: _ProjectReportContext) -> None:
        current_row = self._title_end_row + 2
        for group_index, group in enumerate(self.groups):
            if group_index > 0:
                current_row += 1
            table_result = self.build_tables(ctx, group, start_row=current_row)
            current_row = self.build_charts(ctx, group, table_result)
    ...
    class GroupBuildResult:
        ...
        table_end_row: int
        data_end_col: Optional[int] = None
        chart_anchor_row: Optional[int] = None
        layout: Optional["GroupLayout"] = None
        sections: Optional[list[dict[str, object]]] = None

    def build_tables(self, ctx: _ProjectReportContext, group: ScenarioGroup, *, start_row: int) -> "ProjectReportBuilder.GroupBuildResult":
        ...
        return self._strategy.build_group_tables(self, group=group, header_row=header_row)

    def build_charts(
        self,
        ctx: _ProjectReportContext,
        group: ScenarioGroup,
        table_result: "ProjectReportBuilder.GroupBuildResult",
    ) -> int:
        return self._strategy.build_group_charts(self, group=group, table_result=table_result)

    def save(self, ctx: _ProjectReportContext) -> Path:
        output = Path(self.output_path)
        output.parent.mkdir(parents=True, exist_ok=True)
        self.workbook.save(output)
        LOGGER.info("Project report saved to %s (sheet=%s)", output, self.sheet.title)
