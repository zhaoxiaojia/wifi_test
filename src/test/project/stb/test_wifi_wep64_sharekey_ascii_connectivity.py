# test_wifi_wep64_ascii_connectivity.py
"""
Test Plan: Connect to OPEN-WEP-64bit (ascii) AP and verify connectivity.

Precondition:
1. WiFi is ON on DUT, not connected to any network.

Steps:
1. Configure router 2.4G with WEP-64bit (ascii) encryption.
...
TCID = "WiFi-STA-FSM0008"
WEP_KEY = "ABCDE"  # 10-digit ascii key for WEP-64


@allure.title("Wi-Fi 2.4GHz WEP-64bit (ascii) Connectivity Test")
@allure.description("""
1. Configure 2.4G Wi-Fi with WEP-64bit (ascii) encryption on router.
...
""")
def test_wifi_wep64_ascii_connectivity(wifi_adb_device):
    ...
    try:
        for band in band_list:
            ...
            with allure.step(f"Configure AP {band} with WEP-64bit (ascii) security"):
