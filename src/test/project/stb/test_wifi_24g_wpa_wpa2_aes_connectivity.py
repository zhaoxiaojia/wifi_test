# test_wifi_24g_wpa_wpa2_aes_connectivity.py
"""
...
TCID = "WiFi-STA-FSM00211"
PASSWORD = "88888888"  # 10-digit hex key for WEP-64


@allure.title("Wi-Fi 2.4G WPA_WPA2-PSK&AES Connectivity Test")
@allure.description("""
1. Configure Wi-Fi 2.4G with WPA_WPA2-PSK&AES  encryption on router.
2. Connect DUT to the WEP network via UI with the correct password.
3. Verify internet connectivity by playing an online video.
""")
def test_wifi_24g_wpa_wpa2_aes_connectivity(wifi_adb_device):
    ...
    try:
        for band in band_list:
            ...
            with allure.step(f"Configure AP {band} with WPA_WPA2-PSK&AES security"):
            ...
            with allure.step("Connect DUT to WPA_WPA2-PSK&AES network via UI"):
                ...
                record_test_step(
                    TCID,
                    f"Connect to WPA_WPA2-PSK&AES {ssid_list[band]}",
                    "PASS" if connected else "FAIL",
                    f"RSSI={rssi}"
                )
                assert connected, f"Failed to connect to WPA_WPA2-PSK&AES: {ssid_list[band]}"
            ...
            with allure.step("Play online video to verify internet connectivity"):
                ...
                assert network_works, "Internet connectivity verification failed on WPA_WPA2-PSK&AES network!"
