# test_wifi_5g_80211ax_channel100_bandwidth_switching.py
import pytest
import allure
import time
import logging
...
TCID = "WiFi-STA-FBW0003"
MAX_WAIT_TIME = 60
TARGET_BANDWIDTH = ['20MHZ', '40MHZ', '80MHZ']

@allure.title("Wi-Fi 5GHz 802.11ax Channel 100 Bandwidth Switching (20/40/80MHz) Connectivity Test")
@allure.description("""
1. Configure 5G Wi-Fi on channel 100 with 802.11ax mode via Telnet.
...
""")
def test_wifi_5g_80211ax_ch100_bw_switching_connectivity(wifi_adb_device):
    ...
    try:
        # === Step 1: 配置路由器为 5GHz 802.11ax 模式，并设置信道为 100 ===
        with allure.step("Configure 5G Wi-Fi as 802.11ax on channel 100"):
            configure_ap_wireless_mode(router, band='5g', mode='ax-mixed', ssid=ssid, password=password)
            # 初始频宽设为20MHz
            router.set_5g_channel_bandwidth(channel=100, bandwidth='20/40/80/160MHZ')
            router.commit()


        # === Step 2: 验证AP配置 ===
        with allure.step("Verify AP is in expected 802.11ax mode on channel 100"):
            ...
            logging.info("AP is in expected 802.11ax mode on channel 100: %s", is_valid)
            if not is_valid:
                error_msg = (
                    f"AP did not enter 802.11ax mode or is not on channel 100 as expected.\n"
        ...
        with allure.step("Connect DUT to initial 802.11ax (5G) network on channel 100 (20MHz)"):
            ...
            record_test_step(TCID, f"Initial connect to {ssid} on CH100 (Auto)", "PASS" if connected else "FAIL", f"RSSI={rssi}")
        ...
        for bw in target_bandwidths:
            with allure.step(f"Change AP bandwidth to {bw} and verify DUT reconnection"):
                # 1. 配置路由器到新频宽
                router.set_5g_channel_bandwidth(channel=100, bandwidth=bw)
                router.commit()
            ...
            with allure.step("Play online video to verify internet"):
                ...
                record_test_step(TCID, f"802.11ax (5G) {ssid} on CH100 network works well", "PASS" if network_works else "FAIL", f"Network work well")
