# test_wifi_5g_80211ax_channel132_connectivity.py
import pytest
import allure
import time
import logging
...
TCID = "WiFi-STA-FCH0048"


@allure.title("Wi-Fi 5GHz Channel 132 with 802.11ax Mode Connectivity Test")
@allure.description("""
1. Configure 5G Wi-Fi on channel 132 with 802.11ax mode via Telnet
2. Enable Wi-Fi radio
3. Connect DUT to the 5GHz network on channel 132
4. Play online video to verify internet connectivity
""")
def test_wifi_5g_80211ax_channel132_connectivity(wifi_adb_device):
    ...
    try:
        # === Step 1: 配置路由器为 5GHz 802.11ax 模式，并设置信道为 132 ===
        with allure.step("Configure 5G Wi-Fi as 802.11ax on channel 132"):
            # 1. 先设置信道为 132
        ...
        with allure.step("Verify AP is in expected 802.11ax mode on channel 132"):
            # 首先验证无线模式
            ...
            logging.info("AP is in expected 802.11ax mode on channel 132: %s", is_valid)
        ...
        # === Step 3: DUT 连接 Wi-Fi ===
        with allure.step("Connect DUT to 802.11ax (5G) network on channel 132"):
            ...
            record_test_step(TCID, f"Connect to 802.11ax (5G) {ssid} on CH132", "PASS" if connected else "FAIL",
                             f"RSSI={rssi}")
        ...
        with allure.step("Play online video to verify internet"):
            ...
            record_test_step(TCID, f"802.1ax (5G) {ssid} on CH132 network work well",
