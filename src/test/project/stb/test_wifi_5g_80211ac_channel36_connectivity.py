# test_wifi_5g_80211ac_channel36_connectivity.py
import pytest
import allure
import time
import logging
...
TCID = "WiFi-STA-FCH0029"


@allure.title("Wi-Fi 5GHz Channel 36 with 802.11an-ac-mixed Mode Connectivity Test")
@allure.description("""
1. Configure 5G Wi-Fi on channel 36 with 802.11an-ac-mixed mode via Telnet
2. Enable Wi-Fi radio
3. Connect DUT to the 5GHz network on channel 36
4. Play online video to verify internet connectivity
""")
def test_wifi_5g_channel36_80211ac_connectivity(wifi_adb_device):
    ...
    try:
        # === Step 1: 配置路由器为 5GHz 802.11an-ac-mixed 模式，并设置信道为 36 ===
        with allure.step("Configure 5G Wi-Fi as 802.11an-ac-mixed on channel 36"):
            # 1. 先设置信道为 36
            configure_ap_channel(router, band='5g', channel=36)
        ...
        # === Step 2: 验证是否为 an-only 并且在信道 36（关键步骤，失败则中止）===
        with allure.step("Verify AP is in expected 802.11an-ac-mixed mode on channel 36"):
            # 首先验证无线模式
            is_mode_valid = verify_ap_wireless_mode(router, band='5g', expected_ssid=ssid, expected_mode='an-ac-mixed')
            # 然后验证信道
            is_channel_valid = verify_ap_channel_and_beacon(router, band='5g', expected_channel=36, expected_ssid=ssid)

            is_valid = is_mode_valid and is_channel_valid
            logging.info("AP is in expected 802.11an-ac-mixed mode on channel 36: %s", is_valid)

            if not is_valid:
                error_msg = (
                    f"AP did not enter 802.11ac-only mode or is not on channel 36 as expected.\n"
        ...
        # === Step 3: DUT 连接 Wi-Fi ===
        with allure.step("Connect DUT to 802.11an-ac-mixed (5G) network on channel 36"):
            ...
            record_test_step(TCID, f"Connect to 802.11an-ac-mixed (5G) {ssid} on CH36", "PASS" if connected else "FAIL",
                             f"RSSI={rssi}")
        ...
        with allure.step("Play online video to verify internet"):
            ...
            record_test_step(TCID, f"802.1an-ac-mixed (5G) {ssid} on CH36 network work well",
