# test_wifi_5g_80211ax_connectivity.py
import pytest
import allure
import time
import logging
...
# 请替换为你的实际 TCID
TCID = "WiFi-STA-FMD0012"

@allure.title("Wi-Fi 802.11ax (5GHz) Connectivity Test")
@allure.description("""
1. Configure 5G Wi-Fi as 802.11ax (ax-mixed) via Telnet
2. Enable Wi-Fi radio
3. Connect DUT to the 802.11ax (5G) network
4. Play online video to verify internet connectivity
""")
def test_wifi_5g_80211ax_connectivity(wifi_adb_device):
    ...
    try:
        # === Step 1: 配置路由器为 802.11ax (ax-mixed) (5G) ===
        with allure.step("Configure 5G Wi-Fi as 802.11ax (ax-mixed)"):
        ...
        with allure.step("Verify AP is in expected 802.11ax (ax-mixed) (5G) mode"):
            is_valid = verify_ap_wireless_mode(router, band='5g', expected_ssid=ssid, expected_mode='ax-mixed')
            logging.info("AP is in expected 802.11ax (ax-mixed) (5G) mode: %s", is_valid)

            if not is_valid:
                error_msg = (
                    f"AP did not enter 802.11ax (ax-mixed) mode on 5G as expected.\n"
        ...
        # === Step 3: DUT 连接 Wi-Fi ===
        with allure.step("Connect DUT to 802.11ax (5G) network"):
            ...
            record_test_step(TCID, f"Connect to 802.11ax (5G) {ssid}", "PASS" if connected else "FAIL", f"RSSI={rssi}")
        ...
        with allure.step("Play online video to verify internet"):
            ...
            record_test_step(TCID, f"802.11ax (5G) {ssid} network work well", "PASS" if network_works else "FAIL", f"Network work well")
