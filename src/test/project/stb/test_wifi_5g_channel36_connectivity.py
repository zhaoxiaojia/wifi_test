# test_wifi_5g_channel36_connectivity.py
import pytest
import allure
import time
import logging
...
TCID = "WiFi-STA-FCH0007"

@allure.title("Wi-Fi 5GHz Channel 36 Connectivity Test")
@allure.description("""
1. Configure 5G Wi-Fi on channel 36 via Telnet
2. Enable Wi-Fi radio
3. Connect DUT to the 5GHz network on channel 36
4. Play online video to verify internet connectivity
""")
def test_wifi_5g_channel36_connectivity(wifi_adb_device):
    ...
    try:
        # === Step 1: 配置路由器为 5GHz 默认模式，信道36 ===
        with allure.step("Configure 5G Wi-Fi on channel 36"):
            # 调用驱动函数，指定 band='5g' 和 channel=36
            configure_ap_channel(router, band='5g', channel=36, ssid=ssid, password=password)

        # === Step 2: 验证是否配置成功（关键步骤，失败则中止）===
        with allure.step("Verify AP is configured on channel 36"):
            is_valid = verify_ap_channel_and_beacon(router, band='5g', expected_channel=36, expected_ssid=ssid)
            logging.info("AP is configured on channel 36: %s", is_valid)
            if not is_valid:
                error_msg = (
                    f"AP did not configure properly on channel 36.\n"
                    f"Router may not support this configuration or setup failed."
                )
                logging.error(error_msg)
                allure.attach(body=error_msg, name="AP Channel 36 Configuration Failed", attachment_type=allure.attachment_type.TEXT)
                record_test_step(TCID, f"Configurate AP Channel 36", "PASS" if is_valid else "FAIL", "AP Channel 36 Setting")
                pytest.fail("AP channel 36 configuration verification failed – aborting test.")

        # === Step 3: DUT 连接 Wi-Fi ===
        with allure.step("Connect DUT to 5GHz network on channel 36"):
            ...
            record_test_step(TCID, f"Connect to 5GHz {ssid} on channel 36", "PASS" if connected else "FAIL", f"RSSI={rssi}")
        ...
        with allure.step("Play online video to verify internet"):
            ...
            record_test_step(TCID, f"5GHz {ssid} network work well on channel 36", "PASS" if network_works else "FAIL", f"Network work well")
