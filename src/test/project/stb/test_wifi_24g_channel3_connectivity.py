# test_wifi_2g_channel3_connectivity.py
import pytest
import allure
import time
import logging
...
TCID = "WiFi-STA-FCH0002"


@allure.title("Wi-Fi 2.4GHz Channel 3 Connectivity Test")
@allure.description("""
1. Configure 2.4G Wi-Fi on channel 3 via Telnet
2. Enable Wi-Fi radio
3. Connect DUT to the 2.4GHz network on channel 3
4. Play online video to verify internet connectivity
""")
def test_wifi_2g_channel3_connectivity(wifi_adb_device):
    ...
    try:
        ...
        with allure.step("Configure 2.4G Wi-Fi on channel 3"):
            # 调用驱动函数，指定 band='2g' 和 mode='auto'（默认模式）
            # 注意：configure_ap_wireless_mode 函数需要扩展以支持信道设置
            configure_ap_channel(router, band='2g', channel=3, ssid=ssid, password=password)

        # === Step 2: 验证是否配置成功（关键步骤，失败则中止）===
        with allure.step("Verify AP is configured on channel 3"):
            is_valid = verify_ap_channel_and_beacon(router, band='2g', expected_channel=3, expected_ssid=ssid)
            logging.info("AP is configured on channel 3: %s", is_valid)
            if not is_valid:
                error_msg = (
                    f"AP did not configure properly on channel 3.\n"
                    f"Router may not support this configuration or setup failed."
                )
                logging.error(error_msg)
                allure.attach(body=error_msg, name="AP Channel 3 Configuration Failed",
                              attachment_type=allure.attachment_type.TEXT)
            record_test_step(TCID, f"Configurate AP Channel 3",  "PASS" if is_valid else "FAIL",
                                 "AP Channel 3 Setting")
            if not is_valid:
                pytest.fail("AP channel 3 configuration verification failed – aborting test.")

        # === Step 3: DUT 连接 Wi-Fi ===
        with allure.step("Connect DUT to 2.4GHz network on channel 3"):
            ...
            record_test_step(TCID, f"Connect to 2.4GHz {ssid} on channel 3", "PASS" if connected else "FAIL",
                             f"RSSI={rssi}")
        ...
        with allure.step("Play online video to verify internet"):
            ...
            record_test_step(TCID, f"2.4GHz {ssid} network work well on channel 3", "PASS" if network_works else "FAIL",
                             f"Network work well")
