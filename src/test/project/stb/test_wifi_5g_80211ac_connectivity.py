# test_wifi_5g_80211ac_connectivity.py
import pytest
import allure
import time
import logging
...
# 请替换为你的实际 TCID
TCID = "WiFi-STA-FMD0011"

@allure.title("Wi-Fi 802.11ac (5GHz) Only Mode Connectivity Test")
@allure.description("""
1. Configure 5G Wi-Fi as 802.11ac (an-ac-mixed) via Telnet
2. Enable Wi-Fi radio
3. Connect DUT to the 802.11ac (5G) network
4. Play online video to verify internet connectivity
""")
def test_wifi_5g_80211ac_connectivity(wifi_adb_device):
    ...
    try:
        # === Step 1: 配置路由器为 802.11ac-only (5G) ===
        with allure.step("Configure 5G Wi-Fi as 802.11ac(an-ac-mixed)"):
            # 调用驱动函数，指定 band='5g' 和 mode='ac-only'
            configure_ap_wireless_mode(router, band='5g', mode='an-ac-mixed', ssid=ssid, password=password)

        # === Step 2: 验证是否为 ac-only（关键步骤，失败则中止）===
        with allure.step("Verify AP is in expected 802.11ac-only (5G) mode"):
            is_valid = verify_ap_wireless_mode(router, band='5g', expected_ssid=ssid, expected_mode='an-ac-mixed')
            logging.info("AP is in expected 802.11ac-only (5G) mode: %s", is_valid)

            if not is_valid:
                error_msg = (
                    f"AP did not enter 802.11ac-only mode on 5G as expected.\n"
        ...
        # === Step 3: DUT 连接 Wi-Fi ===
        with allure.step("Connect DUT to 802.11ac (5G) network"):
            ...
            record_test_step(TCID, f"Connect to 802.11ac (5G) {ssid}", "PASS" if connected else "FAIL", f"RSSI={rssi}")
            assert connected, f"Failed to connect to 802.11ac (5G) SSID: {ssid}"
        ...
        with allure.step("Play online video to verify internet"):
            ...
            record_test_step(TCID, f"802.11ac (5G) {ssid} network work well", "PASS" if network_works else "FAIL", f"Network work well")
