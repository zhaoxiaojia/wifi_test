# test_wifi_5g_80211ac_channel120_connectivity.py
import pytest
import allure
import time
import logging
...
TCID = "WiFi-STA-FCH0034"


@allure.title("Wi-Fi 5GHz Channel 120 with 802.11an-ac-mixed Mode Connectivity Test")
@allure.description("""
1. Configure 5G Wi-Fi on channel 120 with 802.11an-ac-mixed mode via Telnet
2. Enable Wi-Fi radio
3. Connect DUT to the 5GHz network on channel 120
4. Play online video to verify internet connectivity
""")
def test_wifi_5g_channel120_80211ac_connectivity(wifi_adb_device):
    ...
    try:
        # === Step 1: 配置路由器为 5GHz 802.11an-ac-mixed 模式，并设置信道为 120 ===
        with allure.step("Configure 5G Wi-Fi as 802.11an-ac-mixed on channel 120"):
            # 1. 先设置信道为 120
            configure_ap_channel(router, band='5g', channel=120)
        ...
        # === Step 2: 验证是否为 an-only 并且在信道 120（关键步骤，失败则中止）===
        with allure.step("Verify AP is in expected 802.11an-ac-mixed mode on channel 120"):
            # 首先验证无线模式
            is_mode_valid = verify_ap_wireless_mode(router, band='5g', expected_ssid=ssid, expected_mode='an-ac-mixed')
            # 然后验证信道
            is_channel_valid = verify_ap_channel_and_beacon(router, band='5g', expected_channel=120, expected_ssid=ssid)

            is_valid = is_mode_valid and is_channel_valid
            logging.info("AP is in expected 802.11an-ac-mixed mode on channel 120: %s", is_valid)

            if not is_valid:
                error_msg = (
                    f"AP did not enter 802.11ac-only mode or is not on channel 120 as expected.\n"
        ...
        # === Step 3: DUT 连接 Wi-Fi ===
        with allure.step("Connect DUT to 802.11an-ac-mixed (5G) network on channel 120"):
            ...
            record_test_step(TCID, f"Connect to 802.11an-ac-mixed (5G) {ssid} on CH120", "PASS" if connected else "FAIL",
                             f"RSSI={rssi}")
        ...
        with allure.step("Play online video to verify internet"):
            ...
            record_test_step(TCID, f"802.1an-ac-mixed (5G) {ssid} on CH120 network work well",
