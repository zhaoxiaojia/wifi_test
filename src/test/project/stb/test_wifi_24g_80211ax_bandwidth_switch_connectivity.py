# test_wifi_2g_80211ax_bandwidth_switch_connectivity.py
...
TCID = "WiFi-STA-FBW0006"
MAX_WAIT_TIME = 60
TARGET_BANDWIDTH = ['20MHZ', '40MHZ']

@allure.title("Wi-Fi 2.4GHz 802.11ax Bandwidth Switch (Auto/20M/40M) Connectivity Test")
@allure.description("""
1. Configure 2.4G Wi-Fi with 802.11ax-only mode and Auto Channel on router.
...
""")
def test_wifi_2g_80211ax_bandwidth_switch_connectivity(wifi_adb_device):
    ...
    try:
        # === Step 1: 配置路由器为 2.4GHz 802.11ax-only 模式，并设置信道为 Auto ===
        with allure.step("Configure 2.4G Wi-Fi as 802.11ax-only on Auto Channel"):
        ...
        # === Step 2: 验证 AP 配置 ===
        with allure.step("Verify AP is in expected 802.11ax-only mode on Auto Channel"):
            is_mode_valid = verify_ap_wireless_mode(router, band='2g', expected_ssid=ssid, expected_mode='ax-only')
            ...
            record_test_step(TCID, "AP config for 2.4G axn Auto", "PASS" if is_valid else "FAIL", "24g_11AX_Auto")
                #pytest.fail("AP configuration verification failed.")

        # === Step 3: DUT 连接 Wi-Fi (Auto Channel) ===
        with allure.step("Connect DUT to 802.11ax (2.4G) network on Auto Channel"):
            ...
            record_test_step(TCID, f"Connect to 802.11ax (2.4G) {ssid} on Auto CH", "PASS" if connected else "FAIL",
                             f"RSSI={rssi}")
            #assert connected, f"Failed to connect to 802.11ax (2.4G) SSID: {ssid}"
