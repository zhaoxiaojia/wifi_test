# test_wifi_80211g_connectivity.py
import pytest
import allure
import time
import logging
...
TCID = "WiFi-STA-FMD0002"  # 建议换一个 TCID

@allure.title("Wi-Fi 2.4G 802.11g Mode Connectivity Test")
@allure.description("""
1. Configure 2.4G Wi-Fi as 802.11g-only via Telnet
2. Enable Wi-Fi radio
3. Connect DUT to the 802.11g network
4. Play online video (verify internet)
""")
def test_wifi_24g_80211g_connectivity(wifi_adb_device):
    ...
    try:
        # === Step 1: 配置路由器为 802.11g-only (2.4G) ===
        with allure.step("Configure 2.4G Wi-Fi as 802.11g-only"):
            configure_ap_wireless_mode(router, band='2g', mode='g-only', ssid=ssid, password=password)

        # === Step 2: 验证是否为 g-only（关键步骤，失败则中止）===
        with allure.step("Verify AP is in expected 802.11g-only mode"):
            is_valid = verify_ap_wireless_mode(router, band='2g', expected_ssid=ssid, expected_mode='g-only')
            logging.info("AP is in expected 802.11g-only mode: %s", is_valid)

            if not is_valid:
                error_msg = (
                    f"AP did not enter 802.11g-only mode as expected.\n"
        ...
        # === Step 3: DUT 连接 Wi-Fi ===
        with allure.step("Connect DUT to 802.11g network"):
            ...
            record_test_step(TCID, f"Connect to 802.11g {ssid}", "PASS" if connected else "FAIL", f"RSSI={rssi}")
            assert connected, f"Failed to connect to 802.11g SSID: {ssid}"
        ...
        with allure.step("Play online video to verify internet"):
            ...
            record_test_step(TCID, f"802.11g {ssid} network work well", "PASS" if network_works else "FAIL",
                             "Network work well")
