# test_wifi_80211bg_mixed_connectivity.py
import pytest
import allure
import time
import logging
...
TCID = "WiFi-STA-FMD0004"

@allure.title("Wi-Fi 802.11b/g Mixed Mode Connectivity Test (2.4GHz)")
@allure.description("""
1. Configure 2.4G Wi-Fi as 802.11b/g mixed via Telnet
2. Enable Wi-Fi radio
3. Connect DUT to the 802.11b/g mixed network
4. Play online video to verify internet connectivity
""")
def test_wifi_24g_80211bg_mixed_connectivity(wifi_adb_device):
    ...
    try:
        # === Step 1: 配置路由器为 802.11b/g mixed (2.4G) ===
        with allure.step("Configure 2.4G Wi-Fi as 802.11b/g mixed"):
            configure_ap_wireless_mode(router, band='2g', mode='bg-mixed', ssid=ssid, password=password)

        # === Step 2: 验证是否为 b/g mixed（关键步骤，失败则中止）===
        with allure.step("Verify AP is in expected 802.11b/g mixed mode"):
            is_valid = verify_ap_wireless_mode(router, band='2g', expected_ssid=ssid, expected_mode='bg-mixed')
            logging.info("AP is in expected 802.11b/g mixed mode: %s", is_valid)

            if not is_valid:
                error_msg = (
                    f"AP did not enter 802.11b/g mixed mode as expected.\n"
        ...
        # === Step 3: DUT 连接 Wi-Fi ===
        with allure.step("Connect DUT to 802.11b/g mixed network"):
            ...
            record_test_step(TCID, f"Connect to 802.11b/g {ssid}", "PASS" if connected else "FAIL", f"RSSI={rssi}")
            assert connected, f"Failed to connect to 802.11b/g SSID: {ssid}"
        ...
        with allure.step("Play online video to verify internet"):
            ...
            record_test_step(TCID, f"802.11b/g {ssid} network works", "PASS" if network_works else "FAIL",
                             "Internet OK")
