# test_wifi_5g_80211ac_ht20_channel165_connectivity.py
import pytest
import allure
import time
import logging
...
TCID = "WiFi-STA-FCH0040"  # 请根据您的实际用例编号规则调整


@allure.title("Wi-Fi 5GHz Channel 165 with 802.11an-ac-mixed and HT20 Mode Connectivity Test")
@allure.description("""
1. Configure 5G Wi-Fi on channel 165 with 802.11an-ac-mixed mode via Telnet
2. Enable Wi-Fi radio
3. Connect DUT to the 5GHz network on channel 165
4. Play online video to verify internet connectivity
""")
def test_wifi_5g_channel165_80211ac_connectivity(wifi_adb_device):
    ...
    try:
        # === Step 1: 配置路由器为 5GHz 802.11an-ac-mixed 模式，并设置信道为 165 ===
        with allure.step("Configure 5G Wi-Fi as 802.11an-ac-mixed on channel 165"):
        ...
        with allure.step("Verify AP is in expected 802.11an-ac-mixed mode on channel 165"):
            # 首先验证无线模式
            ...
            logging.info("AP is in expected 802.11an-ac-mixed mode on channel 165: %s", is_valid)

            if not is_valid:
                error_msg = (
                    f"AP did not enter 802.11ac-only mode or is not on channel 165 as expected.\n"
        ...
        # === Step 3: DUT 连接 Wi-Fi ===
        with allure.step("Connect DUT to 802.11an-ac-mixed (5G) network on channel 165"):
            ...
            record_test_step(TCID, f"Connect to 802.11an-ac-mixed (5G) {ssid} on CH165", "PASS" if connected else "FAIL",
                             f"RSSI={rssi}")
        ...
        with allure.step("Play online video to verify internet"):
            ...
            record_test_step(TCID, f"802.1an-ac-mixed (5G) {ssid} on CH165 network work well",
