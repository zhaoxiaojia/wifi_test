# test_wifi_wep128_hex_connectivity.py
"""
Test Plan: Connect to OPEN-WEP-128bit (Hex) AP and verify connectivity.

Precondition:
1. WiFi is ON on DUT, not connected to any network.

Steps:
1. Configure router 2.4G with WEP-128bit (Hex) encryption.
...
TCID = "WiFi-STA-FSM0007"
WEP_KEY = "ABCDEFGHIGKLM"


@allure.title("Wi-Fi 2.4GHz WEP-128bit (Hex) Connectivity Test")
@allure.description("""
1. Configure 2.4G Wi-Fi with WEP-128bit (Hex) encryption on router.
...
""")
def test_wifi_wep128_hex_connectivity(wifi_adb_device):
    ...
    try:
        for band in band_list:
            ...
            with allure.step(f"Configure AP {band} with WEP-128bit (Hex) security"):
