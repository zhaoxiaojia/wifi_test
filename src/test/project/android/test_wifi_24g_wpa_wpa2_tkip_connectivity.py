# test_wifi_24g_wpa_wpa2_tkip_connectivity.py
"""
...
TCID = "WiFi-STA-FSM0016"
PASSWORD = "88888888"


@allure.title("Wi-Fi 24G WPA_WPA2-PSK&AES+TKIP Connectivity Test")
@allure.description("""
1. Configure Wi-Fi 24G with WPA_WPA2-PSK&AES+TKIP  encryption on router.
2. Connect DUT to the WPA_WPA2 network via UI with the correct password.
3. Verify internet connectivity by playing an online video.
""")
def test_wifi_2g_wpa_wpa2_tkip_connectivity(wifi_adb_device):
    ...
    try:
        for band in band_list:
            ...
            with allure.step(f"Configure AP {band} with WPA_WPA2-PSK&AES+TKIP security"):
            ...
            with allure.step("Connect DUT to WPA_WPA2-PSK&AES+TKIP network via UI"):
                ...
                record_test_step(
                    TCID,
                    f"Connect to WPA_WPA2-PSK&AES+TKIP {ssid_list[band]}",
                    "PASS" if connected else "FAIL",
                    f"RSSI={rssi}"
                )
                assert connected, f"Failed to connect to WPA_WPA2-PSK&AES+TKIP: {ssid_list[band]}"
            ...
            with allure.step("Play online video to verify internet connectivity"):
                ...
                assert network_works, "Internet connectivity verification failed on WPA_WPA2-PSK&AES+TKIP network!"
