# test_wifi_wep128_ascii_connectivity.py
"""
Test Plan: Connect to OPEN-WEP-128bit (ascii) AP and verify connectivity.

Precondition:
1. WiFi is ON on DUT, not connected to any network.

Steps:
1. Configure router 2.4G with WEP-128bit (ascii) encryption.
2. On DUT: Settings -> Network & Internet -> Add/Connect to the WEP network with correct 10-digit ascii key.
...
TCID = "WiFi-STA-FSM0009"
WEP_KEY = "1234567890123"  # 10-digit ascii key for WEP-128


@allure.title("Wi-Fi 2.4GHz WEP-128bit (ascii) Connectivity Test")
@allure.description("""
1. Configure 2.4G Wi-Fi with WEP-128bit (ascii) encryption on router.
2. Connect DUT to the WEP network via UI with the correct 10-digit ascii key.
3. Verify internet connectivity by playing an online video.
""")
def test_wifi_wep128_ascii_connectivity(wifi_adb_device):
    ...
    try:
        for band in band_list:
            # === Step 1: Configure Router for WEP-128 (2.4G only) ===
            with allure.step(f"Configure AP {band} with WEP-128bit (ascii) security"):
            ...
            with allure.step("Connect DUT to WEP-128 network via UI"):
                ...
                record_test_step(
                    TCID,
                    f"Connect to WEP-128 (2.4G) {ssid_list[band]}",
                    "PASS" if connected else "FAIL",
                    f"RSSI={rssi}"
                )
                assert connected, f"Failed to connect to WEP-128 SSID: {ssid_list[band]}"
            ...
            with allure.step("Play online video to verify internet connectivity"):
                ...
                record_test_step(
                    TCID,
                    "WEP-128 network internet works",
