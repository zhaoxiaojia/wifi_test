# test_wifi_5g_80211ac_bandwidth_switching_connectivity.py
"""
Test Plan: 5G 11ac, HT20/40 auto channel connection check

1. Configure 5G Wi-Fi as 802.11ac-only (an-only) with Auto channel via Telnet.
...
@allure.title("Wi-Fi 5GHz 802.11ac Bandwidth Switching (Auto/20/40MHz) Connectivity Test")
@allure.description("""
1. Configure 5G Wi-Fi with 802.11ac mode and Auto Channel on router.
...
""")
def test_wifi_5g_80211ac_bw_switching_connectivity(wifi_adb_device):
    ...
    try:
        # === Step 1: Configure Router to 5GHz 802.11ac mode with Auto Channel ===
        with allure.step("Configure 5G Wi-Fi as 802.11ac-only with Auto channel"):
            # Use 'an-only' which maps to 11ac in 5G
            configure_ap_wireless_mode(
                router, band='5g', mode='an-ac-mixed', ssid=ssid, password=password
            )
        ...
        with allure.step("Verify AP is in expected 802.11ac mode with Auto channel"):
            is_mode_valid = verify_ap_wireless_mode(
                router, band='5g', expected_ssid=ssid, expected_mode='an-ac-mixed'
            )
            ...
            logging.info("AP is in expected 802.11ac mode: %s", is_mode_valid)
            if not is_mode_valid:
                error_msg = "AP did not enter 802.11ac mode as expected."
                logging.error(error_msg)
                allure.attach(body=error_msg, name="AP Mode Verification Failed",
                              attachment_type=allure.attachment_type.TEXT)
            record_test_step(TCID, "AP 5G 802.11ac mode with Auto channel", "PASS" if is_mode_valid else "FAIL", "AP 5G 802.11ac mode")
                #pytest.fail("AP configuration verification failed.")

        # === Step 3: Initial DUT Connection to Wi-Fi ===
        with allure.step("Connect DUT to initial 802.11ac (5G) network"):
        ...
        for bw in TARGET_BANDWIDTH:
            ...
            with allure.step("Play online video to verify internet"):
                ...
                record_test_step(TCID, f"802.11ac (5G) {ssid} network works well",
