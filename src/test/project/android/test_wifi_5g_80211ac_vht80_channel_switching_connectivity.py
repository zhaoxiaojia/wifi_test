# test_wifi_5g_80211ac_vht80_channel_switching_connectivity.py
"""
Test Plan: 5G 11AC VHT80, [CH36/52/100/149] connection check
"""
...
@allure.title("Wi-Fi 5GHz 802.11ac VHT80 Channel Switching (CH36/52/100/149) Connectivity Test")
@allure.description("""
1. Configure 5G Wi-Fi as 802.11ac-only (an-ac-mixed) with 80MHz bandwidth via Telnet.
...
""")
def test_wifi_5g_80211ac_vht80_ch_switching_connectivity(wifi_adb_device):
    ...
    try:
        # === Step 1: Configure Router to 5GHz 802.11ac mode with 80MHz bandwidth ===
        with allure.step("Configure 5G Wi-Fi as 802.11ac-only with 80MHz bandwidth"):
            # Use 'an-ac-mixed' which maps to 11ac in 5G
            configure_ap_wireless_mode(
        ...
        with allure.step("Verify AP is in expected 802.11ac mode with 80MHz bandwidth"):
            ...
            if not is_mode_valid:
                ...
                #record_test_step(TCID, "AP 5G 802.11ac mode with 80MHz", "PASS" if is_mode_valid else "FAIL", "AP 5G 802.11ac mode")
