# test_wifi_5g_80211ax_channel149_connectivity.py
import pytest
import allure
import time
import logging
...
TCID = "WiFi-STA-FCH0051"


@allure.title("Wi-Fi 5GHz Channel 149 with 802.11ax Mode Connectivity Test")
@allure.description("""
...
""")
def test_wifi_5g_80211ax_channel149_connectivity(wifi_adb_device):
    ...
    try:
        ...
        # === Step 3: DUT 连接 Wi-Fi ===
        with allure.step("Connect DUT to 802.11ax (5G) network on channel 149"):
            ...
            record_test_step(TCID, f"Connect to 802.11ax (5G) {ssid} on CH149", "PASS" if connected else "FAIL",
                             f"RSSI={rssi}")
        ...
        with allure.step("Play online video to verify internet"):
            ...
            record_test_step(TCID, f"802.1ax (5G) {ssid} on CH149 network work well",
