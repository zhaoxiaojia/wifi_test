# test_wifi_5g_wpa2_aes_pmf_disabled_connectivity.py
"""
...
TCID = "WiFi-STA-FSM0037"
PASSWORD = "88888888"  # 10-digit hex key for WEP-64


@allure.title("Wi-Fi 2.4G WPA2-PSK &AES PMF Required Connectivity Test")
@allure.description("""
1. Configure Wi-Fi 2.4G with WPA2-PSK &AES  encryption on router.
2. Connect DUT to the WPA2-PSK network via UI with the correct password.
3. Verify internet connectivity by playing an online video.
""")
def test_wifi_5g_wpa2_aes_pmf_required_connectivity(wifi_adb_device):
    ...
    try:
        for band in band_list:
            ...
            with allure.step("Ensure DUT is in a clean state (WiFi ON, no saved networks)"):
                ...
                for forget_band in band_list:
                    dut._forget_wifi_via_ui(serial, ssid_list[forget_band])
                time.sleep(2)

            # === Step 1: Configure Router for WPA2-PSK&AES  ===
            with allure.step(f"Configure AP {band} with WPA2-PSK&AES and PMF Disabled security"):
            ...
            with allure.step("Connect DUT to WPA2-PSK&AES network via UI"):
                ...
                record_test_step(
                    TCID,
                    f"Connect to WPA2-PSK&AES and PMF Disabled {ssid_list[band]}",
                    "PASS" if connected else "FAIL",
                    f"RSSI={rssi}"
                )
                assert connected, f"Failed to connect to WPA2-PSK&AES and PMF Disabled: {ssid_list[band]}"
            ...
            with allure.step("Play online video to verify internet connectivity"):
                ...
                assert network_works, "Internet connectivity verification failed on WPA2-PSK&AES network!"
