# test_wifi_asia_region_5g_connectivity.py
import pytest
import allure
import time
import logging
...
# 请替换为你的实际 TCID
TCID = "WiFi-STA-REG0012"

TEST_CHANNELS_2G = [1, 6, 11]
TEST_CHANNELS_5G = [36, 100, 140]
MAX_WAIT_TIME = 180

@allure.title("Wi-Fi ASIA Region and 5G Channel Connectivity Test")
@allure.description("""
1. Set AP region to ASIA and verify.
2. For 5G: Test channels [36, 100, 165]. Connect, verify internet, then change channel and check reconnection.
""")
def test_wifi_asia_region_5g_connectivity(wifi_adb_device):
    ...
    try:
        ...
        with allure.step("Configure AP to ASIA Region"):
            try:
                support_channel_list = configure_and_verify_ap_country_code(
                    router=router,
                    country_code="ASIA"
                )
                ap_channel_list = (f"✅ Country code verified as ASIA. "
                ...
                error_msg = (
                    f"AP did not setting ASIA region successfully.\n"
                    f"Exception: {str(e)}"
                )
