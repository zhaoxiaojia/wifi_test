# test_wifi_au_region_5g_connectivity.py
import pytest
import allure
import time
import logging
...
# 请替换为你的实际 TCID
TCID = "WiFi-STA-REG0010"

TEST_CHANNELS_2G = [1, 6, 11]
TEST_CHANNELS_5G = [36, 64, 165]
MAX_WAIT_TIME = 180

@allure.title("Wi-Fi Australia Region and 5G Channel Connectivity Test")
@allure.description("""
1. Set AP region to Australia and verify.
2. For 5G: Test channels [36, 64, 165]. Connect, verify internet, then change channel and check reconnection.
""")
def test_wifi_au_region_5g_connectivity(wifi_adb_device):
    ...
    try:
        # === Step 1: 配置路由器Region为KR) ===
        with allure.step("Configure AP to Australia Region"):
            try:
                support_channel_list = configure_and_verify_ap_country_code(
                    router=router,
                    country_code="AU"
                )
                ap_channel_list = (f"✅ Country code verified as Australia. "
                                   f"5G Channels: {support_channel_list['5g_channels']}")
                record_test_step(TCID, f"AP Australia Region ", "PASS", ap_channel_list)
            except Exception as e:  # 捕获任何异常
                error_msg = (
                    f"AP did not setting Australia region successfully.\n"
                    f"Exception: {str(e)}"
                )
            ...
            with allure.step("Configure 5G Wi-Fi as auto with auto bandwidth"):
            ...
            with allure.step("Connect DUT to initial auto (5G) network"):
                ...
                record_test_step(TCID, f"Initial connect to {ssid_5g} (CH Auto))",
            ...
            for ch in TEST_CHANNELS_5G:
                ...
                with allure.step("Play online video to verify internet"):
                    ...
                    record_test_step(TCID, f"Auto (5G) {ssid_5g} network works well",
