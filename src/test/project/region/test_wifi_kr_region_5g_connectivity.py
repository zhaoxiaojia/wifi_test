# test_wifi_kr_region_5g_connectivity.py
import pytest
import allure
import time
import logging
...
# 请替换为你的实际 TCID
TCID = "WiFi-STA-REG0008"

TEST_CHANNELS_2G = [1, 6, 11]
TEST_CHANNELS_5G = [36, 100, 165]
MAX_WAIT_TIME = 180

@allure.title("Wi-Fi Korea Region and 5G Channel Connectivity Test")
@allure.description("""
1. Set AP region to Korea and verify.
2. For 5G: Test channels [36, 100, 165]. Connect, verify internet, then change channel and check reconnection.
""")
def test_wifi_kr_region_5g_connectivity(wifi_adb_device):
    ...
    try:
        ...
        with allure.step("Configure AP to Korea Region"):
            try:
                support_channel_list = configure_and_verify_ap_country_code(
                    router=router,
                    country_code="KR"
                )
                ap_channel_list = (f"✅ Country code verified as Korea. "
                                   f"5G Channels: {support_channel_list['5g_channels']}")
                record_test_step(TCID, f"AP Korea Region ", "PASS", ap_channel_list)
            except Exception as e:  # 捕获任何异常
                error_msg = (
                    f"AP did not setting Korea region successfully.\n"
                    f"Exception: {str(e)}"
                )
