# test_wifi_jp_region_24g_connectivity.py
import pytest
import allure
import time, os
import logging
...
# 请替换为你的实际 TCID
TCID = "WiFi-STA-REG0009"

TEST_CHANNELS_2G = [1, 6, 11, 14]
TEST_CHANNELS_5G = [36, 64, 100, 140]
MAX_WAIT_TIME = 380
OLD_PASSWORD = "88888881"
TEST_24G_FIX_REGION_CODE = "JP"
TEST_5G_FIX_REGION_CODE = "US"
TEST_REGION_CODE = "JP"
TEST_REGION_CODE2 = "JP(日本)"
...
2.For 2.4G: Test channels [1, 6, 11, 14]. Connect, verify internet, then change channel and check reconnection.
""")
def test_wifi_jp_24g_region_connectivity(wifi_adb_device):
    ...
    try:
        ...
        with allure.step("Configure DUT to {TEST_REGION_CODE2} Region"):
            try:
                ap_support_channel_list = configure_and_verify_ap_country_code(
                    router=router,
                    country_code=TEST_24G_FIX_REGION_CODE,
                ...
                ap_channel_list = (f"{dut_support_channel_list['2g_channels']}")
                record_test_step(TCID, f"Step1. Set DUT region to {TEST_REGION_CODE2}", "PASS", "")
                record_test_step(TCID, f"Step2. Get 2.4G band channel list:", "PASS", ap_channel_list)
                if test_type == "Full Channels":
                    test_channel_list = ap_support_channel_list['2g_channels']
            ...
            # === Step 2: Configure Router to 2.4GHz ===
            with allure.step(f"Configure 2.4G Wi-Fi to {ssid_2g}"):
            ...
            with allure.step("Connect DUT to initial (2.4G) network"):
                ...
                if connected == False:
                    record_test_step(TCID, f"connect to {country} AP {ssid_2g} failed )",
                                     "FAIL", f"RSSI={rssi}")
            ...
            for ch in test_channel_list:
                ...
                with allure.step(f"Change AP channel to {ch} and verify DUT connection"):
                    ...
                    while time.time() - start_time < MAX_WAIT_TIME:
                        ...
                        country = dut.get_wifi_country_code(serial)
                        if current_ssid == ssid_2g and current_channel == ch and country == TEST_REGION_CODE:
                            reconnected = True
                ...
                if reconnected:
                    with allure.step("Play online video to verify internet"):
                        ...
                        if ping_ok:
                            test_result = "Pass"
                collector.add_result(channel=ch, band="2G", country=TEST_REGION_CODE, result=test_result)
        ...
        with allure.step("Restore AP and DUT to default"):
            restore_ap_default_wireless(
                router,
                band='2g',
                original_ssid=wifi_config.get("24g_ssid"),  # 👈 恢复 5G SSID
