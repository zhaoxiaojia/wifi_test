# test_wifi_5g_80211ac_channel100_connectivity.py
import pytest
import allure
import time
import logging
...
TCID = "WiFi-STA-FCH0033"


@allure.title("Wi-Fi 5GHz Channel 100 with 802.11an-ac-mixed Mode Connectivity Test")
@allure.description("""
1. Configure 5G Wi-Fi on channel 100 with 802.11an-ac-mixed mode via Telnet
2. Enable Wi-Fi radio
3. Connect DUT to the 5GHz network on channel 100
4. Play online video to verify internet connectivity
""")
def test_wifi_5g_channel100_80211ac_connectivity(wifi_adb_device):
    ...
    try:
        # === Step 1: 配置路由器为 5GHz 802.11an-ac-mixed 模式，并设置信道为 100 ===
        with allure.step("Configure 5G Wi-Fi as 802.11an-ac-mixed on channel 100"):
            # 1. 先设置信道为 100
            configure_ap_channel(router, band='5g', channel=100)

            # 2. 再设置无线模式为 'an-only' (802.11ac)
            configure_ap_wireless_mode(router, band='5g', mode='an-ac-mixed', ssid=ssid, password=password)



        # === Step 2: 验证是否为 an-only 并且在信道 100（关键步骤，失败则中止）===
        with allure.step("Verify AP is in expected 802.11an-ac-mixed mode on channel 100"):
            # 首先验证无线模式
            is_mode_valid = verify_ap_wireless_mode(router, band='5g', expected_ssid=ssid, expected_mode='an-ac-mixed')
            # 然后验证信道
            is_channel_valid = verify_ap_channel_and_beacon(router, band='5g', expected_channel=100, expected_ssid=ssid)

            is_valid = is_mode_valid and is_channel_valid
            logging.info("AP is in expected 802.11an-ac-mixed mode on channel 100: %s", is_valid)

            if not is_valid:
                error_msg = (
                    f"AP did not enter 802.11ac-only mode or is not on channel 100 as expected.\n"
        ...
        # === Step 3: DUT 连接 Wi-Fi ===
        with allure.step("Connect DUT to 802.11an-ac-mixed (5G) network on channel 100"):
            ...
            record_test_step(TCID, f"Connect to 802.11an-ac-mixed (5G) {ssid} on CH100", "PASS" if connected else "FAIL",
                             f"RSSI={rssi}")
            assert connected, f"Failed to connect to 802.11an-ac-mixed (5G) SSID: {ssid}"
        ...
        with allure.step("Play online video to verify internet"):
            ...
            record_test_step(TCID, f"802.1an-ac-mixed (5G) {ssid} on CH100 network work well",
