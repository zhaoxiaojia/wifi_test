# test_wifi_5g_80211n_channel52_connectivity.py
import pytest
import allure
import time
import logging
...
TCID = "WiFi-STA-FCH0020"  # 请根据您的实际用例编号规则调整


@allure.title("Wi-Fi 5GHz Channel 52 with 802.11n Only Mode Connectivity Test")
@allure.description("""
1. Configure 5G Wi-Fi on channel 52 with 802.11n-only mode via Telnet
2. Enable Wi-Fi radio
3. Connect DUT to the 5GHz network on channel 52
4. Play online video to verify internet connectivity
""")
def test_wifi_5g_channel52_80211n_connectivity(wifi_adb_device):
    ...
    try:
        # === Step 1: 配置路由器为 5GHz 802.11n-only 模式，并设置信道为 52 ===
        with allure.step("Configure 5G Wi-Fi as 802.11n-only on channel 52"):
            # 1. 先设置信道为 52
            configure_ap_channel(router, band='5g', channel=52)
        ...
        with allure.step("Verify AP is in expected 802.11n-only mode on channel 52"):
            # 首先验证无线模式
            ...
            logging.info("AP is in expected 802.11n-only mode on channel 52: %s", is_valid)

            if not is_valid:
                error_msg = (
                    f"AP did not enter 802.11n-only mode or is not on channel 52 as expected.\n"
        ...
        # === Step 3: DUT 连接 Wi-Fi ===
        with allure.step("Connect DUT to 802.11n (5G) network on channel 52"):
            ...
            record_test_step(TCID, f"Connect to 802.11n (5G) {ssid} on CH52", "PASS" if connected else "FAIL",
                             f"RSSI={rssi}")
        ...
        with allure.step("Play online video to verify internet"):
            ...
            record_test_step(TCID, f"802.11n (5G) {ssid} on CH52 network work well",
