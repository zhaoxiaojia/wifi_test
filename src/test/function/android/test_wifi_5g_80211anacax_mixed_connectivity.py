# test_wifi_5g_80211anacax_mixed_connectivity.py
import pytest
import allure
import time
import logging
...
# 请替换为你的实际 TCID
TCID = "WiFi-STA-FMD0015"

@allure.title("Wi-Fi 802.11a/n/ac/ax (5GHz) Mixed Mode Connectivity Test")
@allure.description("""
1. Configure 5G Wi-Fi as 802.11a/n/ac/ax mixed (ax-mixed) via Telnet
2. Enable Wi-Fi radio
3. Connect DUT to the 802.11a/n/ac/ax (5G) network
4. Play online video to verify internet connectivity
""")
def test_wifi_5g_80211anacax_mixed_connectivity(wifi_adb_device):
    ...
    try:
        # === Step 1: 配置路由器为 802.11a/n/ac/ax mixed (ax-mixed) (5G) ===
        with allure.step("Configure 5G Wi-Fi as 802.11a/n/ac/ax mixed (ax-mixed)"):
            # 调用驱动函数，指定 band='5g' 和 mode='ax-mixed'
            configure_ap_wireless_mode(router, band='5g', mode='ax-mixed', ssid=ssid, password=password)

        # === Step 2: 验证是否为 ax-mixed（关键步骤，失败则中止）===
        with allure.step("Verify AP is in expected 802.11a/n/ac/ax mixed (ax-mixed) (5G) mode"):
            is_valid = verify_ap_wireless_mode(router, band='5g',expected_ssid=ssid,  expected_mode='ax-mixed')
            logging.info("AP is in expected 802.11a/n/ac/ax mixed (ax-mixed) (5G) mode: %s", is_valid)

            if not is_valid:
                error_msg = (
                    f"AP did not enter 802.11a/n/ac/ax mixed (ax-mixed) mode on 5G as expected.\n"
        ...
        # === Step 3: DUT 连接 Wi-Fi ===
        with allure.step("Connect DUT to 802.11a/n/ac/ax (5G) network"):
            ...
            record_test_step(TCID, f"Connect to 802.11a/n/ac/ax (5G) {ssid}", "PASS" if connected else "FAIL", f"RSSI={rssi}")
            assert connected, f"Failed to connect to 802.11a/n/ac/ax (5G) SSID: {ssid}"
        ...
        with allure.step("Play online video to verify internet"):
            ...
            record_test_step(TCID, f"802.11a/n/ac/ax (5G) {ssid} network work well", "PASS" if network_works else "FAIL", f"Network work well")
