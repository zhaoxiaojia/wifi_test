# test_wifi_check_connect_status.py
"""
...
TCID = "WiFi-STA-FLS0001"
PASSWORD = "88888888"  # 10-digit hex key for WEP-64


@allure.title("Verify Wi-Fi Connect Status")
@allure.description("""
1. Connect DUT to the AP Wi-Fi network via UI with the correct password.
3. Verify internet connectivity by playing an online video.
""")
def test_wifi_check_connect_status(wifi_adb_device):
    ...
    try:
        for band in band_list:
            ...
            # === Step 1: Connect DUT via UI ===
            with allure.step("Connect DUT to Wi-Fi network via UI"):
                ...
                record_test_step(
                    TCID,
                    f"Connect to {ssid_list[band]}",
                    "PASS" if connected else "FAIL",
                    f"RSSI={rssi}"
                )
                assert connected, f"Failed to connect to {ssid_list[band]}"
            ...
            with allure.step("Play online video to verify internet connectivity"):
                ...
                record_test_step(
                    TCID,
                    "AP Wi-Fi network internet works",
                    "PASS" if network_works else "FAIL",
                    "YouTube playable and ping successful" if network_works else "Connectivity failed"
                )
                assert network_works, "Internet connectivity verification failed on Wi-Fi network!"
