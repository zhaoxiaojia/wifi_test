...
def _wait_for_wifi_association_and_ip(dut, timeout_s=300, check_interval=5):
    """
    等待 DUT 不仅获取到 IP 地址，而且 Wi-Fi 驱动确认已关联到 AP。
    Returns:
    ...
    while time.time() - start_time < timeout_s:
        # 1. 首先检查是否有 IP
        has_ip, ip_addr = dut.wifi_wait_ip()  # 假设存在这个底层方法，或者你可以复用 wifi_wait_ip 的部分逻辑
        if not has_ip:
            time.sleep(check_interval)
            continue

        # 2. 如果有 IP，再检查 Wi-Fi 是否真正关联
        try:
            # 调用 get_rssi 尝试获取连接信息
            dut.get_rssi()
            # 检查关键属性是否被成功设置
            if hasattr(dut, 'bssid') and dut.bssid and dut.bssid != "N/A":
                logging.info(f"✅ Wi-Fi is associated to BSSID: {dut.bssid} with IP: {ip_addr}")
                return True, ip_addr
            else:
                logging.info("⚠️ IP found, but Wi-Fi association not confirmed. Retrying...")
        except Exception as e:
            logging.warning(f"Failed to get RSSI during association check: {e}")

        time.sleep(check_interval)

    logging.error(f"❌ Timed out waiting for Wi-Fi association and IP after {timeout_s} seconds.")
    return False, None
